...
function loadTabs(): TabItem[] {
  ...
  const info = getRouteInfo("/dashboard");
  return [{ path: "/dashboard", title: info.title, icon: info.icon, color: info.color }];
}
...
export function TabsProvider({ children }: { children: ReactNode }) {
  ...
  const [activeTab, setActiveTabState] = useState<string>(() => {
    ...
    return stored[0]?.path || "/dashboard";
  });
  ...
  useEffect(() => {
    ...
    setTabs((prev) => {
      ...
      const baseMatch = prev.find((t) => getBaseRoute(t.path) === baseRoute && t.path !== "/dashboard");
  ...
  const closeTab = useCallback(
    (path: string) => {
      ...
      setTabs((prev) => {
        ...
        if (newTabs.length === 0) {
          const info = getRouteInfo("/dashboard");
          newTabs.push({ path: "/dashboard", title: info.title, icon: info.icon, color: info.color });
        }
        saveTabs(newTabs);
        ...
        if (path === activeTab) {
          ...
          const newActive = newTabs[Math.min(closedIndex, newTabs.length - 1)]?.path || "/dashboard";
