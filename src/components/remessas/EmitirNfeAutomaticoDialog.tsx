...
export function EmitirNfeAutomaticoDialog({
  ...
  const handleEmitir = async () => {
    ...
    try {
      ...
      // Buscar emitente pela inscrição do produtor (cada sócio tem o seu)
      const { data: emitente, error: emitenteError } = await supabase
        .from("emitentes_nfe")
        .select("*")
        .eq("inscricao_produtor_id", inscricao.id)
        .eq("ativo", true)
        .maybeSingle();

      if (emitenteError || !emitente) {
        throw new Error("Emitente NF-e não configurado para esta inscrição do produtor. Cadastre o emitente em FISCAL → Emitentes NF-e vinculado a esta inscrição.");
      }
      ...
      const itensDataForEmission: NotaFiscalItemData[] = [{
        ...
        valor_outros: null,
        // Reforma Tributária
        cst_ibs: itemData.cst_ibs,
        base_ibs: itemData.base_ibs,
        aliq_ibs: itemData.aliq_ibs,
        valor_ibs: itemData.valor_ibs,
        cclass_trib_ibs: itemData.cclass_trib_ibs,
        cst_cbs: itemData.cst_cbs,
        base_cbs: itemData.base_cbs,
