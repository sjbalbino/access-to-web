        ...
        <CardContent className="pt-4 grid grid-cols-2 md:grid-cols-6 gap-3">
          ...
          <div>
            ...
            <Select isSearchable value={filtroFornec} onValueChange={(v) => setFiltroFornec(v === 'all' ? undefined : v)}>
              <Select isSearchableTrigger><Select isSearchableValue placeholder="Todos" /></SelectTrigger>
              <Select isSearchableContent><Select isSearchableItem value="all">Todos</SelectItem>{fornecedores?.map(c => <Select isSearchableItem key={c.id} value={c.id}>{c.nome}</SelectItem>)}</SelectContent>
            </Select>
          </div>
